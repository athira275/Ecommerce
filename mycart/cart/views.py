...
from .models import Cart, Cart_item, Order
...
def add_cart(request, product_id):
    ...
    cart_id = _cart_id(request)  # Get the session cart_id
    ...
    if request.user.is_authenticated:
        ...
        user = None  # Set user to None for anonymous users

    # Retrieve or create a Cart object
    cart, created = Cart.objects.get_or_create(cart_id=cart_id, user=user)

    # Retrieve or create a Cart_item object
    cart_item, created = Cart_item.objects.get_or_create(cart=cart, product=product, user=user)
    
    if created:
        cart_item.quantity = 1  # If the cart item was newly created, set quantity to 1
    else:
        cart_item.quantity += 1  # If it already existed, increment the quantity
